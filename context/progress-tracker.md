...
- [x] 17 Receipts (PDF) + Due/Overdue Tracking + Term Bill PDF — **Phase 11 overhaul complete**: `fee_bundles` table (name/target_class/billing_cycle/term_id/due_date); `fee_bundle_id` FK on `fee_structures`; `receipt_number` indexed string on `fee_payments`; `receipt_prefix` on `school_profile`; `FeeBundle` model with `items()`/`term()`/`academicYear()` relations; `NumberToWords::convert()` helper (GHS Cedis/Pesewas); `ReceiptService` rewritten with `generateReceiptNumber()` (`{PREFIX}{YY}.{MM}.{NN}` sequential), `recordBundlePayment()` (greedy allocation, all items share one receipt_number, DB transaction), `recordStandalonePayment()`, `buildReceipt()` (UUID fallback for legacy), `generatePdf()`; `FeeStatusService` eager-loads `bundle`; `RecordPaymentRequest` accepts `fee_bundle_id` OR `fee_structure_id`; `FeeController` rewritten with bundle CRUD (`storeBundle/updateBundle/destroyBundle/storeBundleItem`), `pay()` dispatches to bundle or standalone, `paystackCheckout/Callback` handle bundles; `ReceiptController::download()` accepts receipt_number string with UUID fallback; `PaystackWebhookController` uses ReceiptService; route `{feePayment}` → `{receiptNumber}`, 4 bundle routes added; `receipt-pdf.blade.php` rebuilt (two A5 halves, bold caps school name, italic motto, Official Receipt, sequential No:, Received From, itemized fee table, amount in words, remaining balance + Full/Partial tag, Accounts Officer + sig/stamp); `term-bill-pdf.blade.php` groups items by bundle with blue header rows; `fees/index.blade.php` collection tab shows bundle cards with "Pay Bundle" + deduplicated receipt links, payment modal supports `fee_bundle_id`; structure tab shows Fee Bundles section with bundle CRUD modals; 4 migrations: `2026_06_30_000002–000005`
