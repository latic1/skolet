...
.copy { padding: 6mm 12mm 6mm 12mm; height: 148.5mm; overflow: hidden; }
...
.sig-line { width: 65%; border-top: 1px solid #333; margin: 8mm auto 1.5mm; }
...
@php
    ...
    // Guardian text sometimes arrives with stray `_x000D_` tokens (an
    // unescaped Excel line-break marker) alongside real \r\n — normalize
    // both into a single clean comma-separated line.
    $guardian = null;
    if ($guardianRaw = ($student->guardian_name ?? null)) {
        $normalized = str_replace(['_x000D_', '_x000A_'], "\n", $guardianRaw);
        $parts      = array_filter(array_map('trim', preg_split('/[\r\n,]+/', $normalized)), fn($p) => $p !== '');
        $guardian   = implode(', ', $parts) ?: null;
    }
