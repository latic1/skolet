...
<div class="flex flex-col gap-6 max-w-4xl">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-base font-semibold text-text-primary">Deleted Staff</h2>
    ...
    <div class="bg-surface border border-border rounded-2xl shadow-card p-12 text-center">
        ...
        <p class="text-sm text-text-muted">No staff members have been deleted.</p>
    </div>
    @else
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="border-b border-border bg-surface-secondary">
                        <th class="text-left text-xs font-medium text-text-muted uppercase tracking-wide px-4 py-3">Name</th>
                        <th class="text-left text-xs font-medium text-text-muted uppercase tracking-wide px-4 py-3 hidden md:table-cell">Role Title</th>
                    ...
                    <tr class="hover:bg-surface-secondary transition-colors">
                        ...
                        <td class="px-4 py-3 text-right">
                            <div class="flex items-center justify-end gap-2">
                                <form method="POST" action="{{ $host }}/staff/{{ $member->id }}/restore">
                                    @csrf
                                    <button type="submit"
                                ...
                                <form method="POST" action="{{ $host }}/staff/{{ $member->id }}/force-delete"
